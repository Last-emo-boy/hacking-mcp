...
async def test_adapter_schema_includes_tool_specific_parameters(registry, safety):
    ...
    assert {"interactive"}.issubset(airgeddon_schema)
    assert not {
        "pmkid", "deauth_count", "capture_file", "target_essid",
    }.intersection(airgeddon_schema)
...
def test_kawaiideauther_source_reviewed_interactive_policy_only(registry, safety):
    from hacking_mcp.mcp_tools.tool_adapters import adapter_parameter_names
    ...
    tool = registry.get_tool("kawaiideauther")

    assert tool.run_command == "cd KawaiiDeauther && sudo bash KawaiiDeauther.sh"
    assert specs["kawaiideauther"].exposed is False
    assert "DANGEROUS" in specs["kawaiideauther"].blocked_reason
    assert records["kawaiideauther"].source_status == "source-reviewed"
    assert records["kawaiideauther"].unverified_parameters == ()
    assert records["kawaiideauther"].gap == ""
    assert any("aryanrtm/KawaiiDeauther" in item for item in records["kawaiideauther"].evidence)

    params = adapter_parameter_names(tool, specs["kawaiideauther"])
    for removed in (
        "interface",
        "bssid",
        "essid",
    ...
    preview = adapter_request_preview(
        tool,
        specs["kawaiideauther"],
        {"target": "ignored.example", "interactive": True},
    )
    assert preview["target"] == ""
    assert preview["options"] == ""
    assert preview["executable"] is False
...
def test_airgeddon_source_reviewed_interactive_policy_only(registry, safety):
    ...
    tool = registry.get_tool("airgeddon")

    assert tool.run_command == "cd airgeddon && sudo bash airgeddon.sh"
    assert specs["airgeddon"].exposed is False
    assert "Wireless Attack" in specs["airgeddon"].blocked_reason
    assert records["airgeddon"].source_status == "source-reviewed"
    assert records["airgeddon"].unverified_parameters == ()
    assert records["airgeddon"].gap == ""
    assert any("v1s1t0r1sh3r3/airgeddon" in item for item in records["airgeddon"].evidence)

    params = adapter_parameter_names(tool, specs["airgeddon"])
    for removed in (
        "interface",
        "bssid",
        "essid",
...
def test_debinject_source_reviewed_interactive_policy_only(registry, safety):
    from hacking_mcp.mcp_tools.tool_adapters import adapter_parameter_names
    ...
    tool = registry.get_tool("debinject")

    assert tool.run_command == "cd Debinject && python debinject.py"
    assert specs["debinject"].exposed is False
    assert "DANGEROUS" in specs["debinject"].blocked_reason
    assert records["debinject"].source_status == "source-reviewed"
    assert records["debinject"].unverified_parameters == ()
    assert records["debinject"].gap == ""
    assert any("UndeadSec/Debinject" in item for item in records["debinject"].evidence)

    params = adapter_parameter_names(tool, specs["debinject"])
    for removed in (
        "payload_type",
        "platform",
        "architecture",
    ...
    preview = adapter_request_preview(
        tool,
        specs["debinject"],
        {"target": "ignored.deb", "interactive": True},
    )
    assert preview["target"] == ""
    assert preview["options"] == ""
    assert preview["executable"] is False
...
def test_lockphish_source_reviewed_interactive_policy_only(registry, safety):
    from hacking_mcp.mcp_tools.tool_adapters import adapter_parameter_names
    ...
    tool = registry.get_tool("lockphish")

    assert tool.run_command == "cd lockphish && bash lockphish.sh"
    assert specs["lockphish"].exposed is False
    assert "classified DANGEROUS" in specs["lockphish"].blocked_reason
    assert records["lockphish"].source_status == "source-reviewed"
    assert records["lockphish"].unverified_parameters == ()
    assert records["lockphish"].gap == ""
    assert any("JasonJerry/lockphish" in item for item in records["lockphish"].evidence)

    params = adapter_parameter_names(tool, specs["lockphish"])
    for removed in (
        "template",
        "landing_url",
        "listener_host",
