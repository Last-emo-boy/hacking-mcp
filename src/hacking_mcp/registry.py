...
from hacking_mcp.environment import detect_environment, ExecBackend
...
class ToolRegistry:
    ...
    def _scan_availability(self) -> None:
        """Check which tools are on PATH and available.

        On Windows with WSL2, scans both host PATH and WSL2 PATH.
        """
        ...
        for name, tool in self._tools.items():
            ...
            # If on Windows and tool is Linux-only, check WSL2 PATH
            if wsl_supported and not native_supported:
                wsl_path = self._which_in_wsl(exe)
                if wsl_path:
                    self._availability[name] = ToolAvailability(
                        tool_name=name,
                        available=True,
    ...
    def _which_in_wsl(self, exe: str) -> Optional[str]:
        """Check if a tool is available inside WSL2."""
        try:
    ...
    def get_availability(self, name: str) -> ToolAvailability:
        """Get availability status for a tool."""
